...
int dxl_hal_set_baud( int socket, float baudrate  )
{
	...
	if(ioctl(socket, TIOCGSERIAL, &serinfo) < 0) {
		fprintf(stderr, "Cannot get serial info\n");
		return 0;
	}
	...
	if(ioctl(socket, TIOCSSERIAL, &serinfo) < 0) {
